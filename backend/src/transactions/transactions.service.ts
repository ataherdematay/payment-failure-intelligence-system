...
export class TransactionsService {
  ...
  private applyFilters(
    ...
    if (dto.failureReason)
      qb.andWhere('t.failure_reason = :fr', { fr: dto.failureReason });
    if (dto.country)
      qb.andWhere('t.country = :country', { country: dto.country });
    if (dto.device) qb.andWhere('t.device = :device', { device: dto.device });
    if (dto.paymentMethod)
      qb.andWhere('t.payment_method = :pm', { pm: dto.paymentMethod });
    if (dto.gateway) qb.andWhere('t.gateway = :gw', { gw: dto.gateway });
    if (dto.dateFrom)
      qb.andWhere('t.created_at >= :from', { from: dto.dateFrom });
  ...
  async seed(count = 5000): Promise<{ inserted: number }> {
    ...
    const REASON_WEIGHTS = [0.35, 0.25, 0.15, 0.15, 0.1];
    ...
    for (let batch = 0; batch < Math.ceil(count / batchSize); batch++) {
      ...
      for (let i = 0; i < batchCount; i++) {
        const createdAt = new Date(
          now.getTime() - rand(0, 90 * 24 * 3600 * 1000),
        );
        ...
        const reason = isFailed
          ? pickWeighted(FAILURE_REASONS, REASON_WEIGHTS)
          : null;
  ...
  async importCsv(
    ...
    for (let i = 0; i < rows.length; i++) {
      try {
        ...
        const riskScore = parseFloat(
          get('risk_score') || get('riskscore') || '0.3',
        );
        ...
        batch.push({
          ...
          paymentMethod:
            get('payment_method') || get('paymentmethod') || 'credit_card',
          gateway: get('gateway') || 'stripe',
          status,
          failureReason: reason ?? undefined,
          retryCount: parseInt(get('retry_count') || '0') || 0,
          riskScore: isNaN(riskScore)
            ? 0.3
            : Math.min(0.99, Math.max(0, riskScore)),
    ...
    this.logger.log(
      `CSV import complete: ${batch.length} inserted, ${errors.length} errors.`,
    );
